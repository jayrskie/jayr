...
$getBookStmt = $conn->prepare('SELECT book_id, user_id, book_title, requested_duration FROM borrow_requests WHERE id = ?');
...
if ($bookRow) {
    ...
    $duration = $bookRow['requested_duration'];
    ...
    $historyStmt = $conn->prepare('INSERT INTO borrow_history (user_id, book_id, book_title, expires_at) VALUES (?, ?, ?, DATE_ADD(NOW(), INTERVAL ? DAY))');
    $historyStmt->bind_param('iisi', $userId, $bookId, $bookTitle, $duration);
    $historyStmt->execute();
    $historyStmt->close();
}
