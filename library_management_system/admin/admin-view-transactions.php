        ...
        try{
            ...
            if (requestedList.length === 0){
                ...
                requestedList.reverse().forEach(r => {
                    const displayUser = r.username || r.library_id || ('#' + (r.user_id || r.userId || r.id));
                    const html = `<div class="request-card" data-request-id="${r.id}">
                        <div class="request-info">
                            <div><strong>${displayUser}</strong> is requesting to borrow the book (<strong>${r.book_title}</strong>) for <strong>${r.requested_duration} days</strong></div>
