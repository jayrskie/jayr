...
<body>
    ...
    async function fetchBorrowedBooks(){
        try{
                ...
                borrowedList.reverse().forEach(r => {
                    const html = `<div class="borrowed-book-card" data-request-id="${r.id}">
                        <div class="borrowed-info">
                            <div><strong>${r.username}</strong> borrowed the book (<strong>${r.book_title}</strong>)</div>
            ...
            // Attach return handlers after a short delay to ensure DOM is updated
            setTimeout(attachReturnHandlers, 100);
        } catch (err){
    ...
    async function handleReturnBook(requestId){
        console.log('Attempting to return book with request ID:', requestId);
        try{
            ...
            if (!res.ok) {
                ...
                alert('Failed to return book: ' + res.status + ' ' + res.statusText);
                return;
            }
            ...
            console.error('handleReturnBook error:', err);
