...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Get the book_id and current status for this copy
    $get_book = $conn->prepare('SELECT book_id, status FROM book_copies WHERE id = ?');
    ...
    // Prevent changing status for copies that are currently borrowed
    if ($current_status === 'borrowed') {
        echo json_encode(['success' => false, 'message' => 'Cannot change status of a borrowed copy']);
        exit();
    }
