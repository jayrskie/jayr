...
try {
    ...
    try {
        ...
        // Early Return: 2+ days (48+ hours) before due date
        // On Time Return: within 1 day (0-24 hours) before due date
        // Late Return: within 1 day (0-24 hours) after due date
        // Overdue: more than 1 day (24+ hours) after due date
        
        if ($hours_diff <= -48) {
            // Returned 2+ days early
            $return_status = 'early';
        } elseif ($hours_diff > -48 && $hours_diff < 0) {
            // Returned between 2 days and 0 hours before due date
            $return_status = 'on_time';
        } elseif ($hours_diff >= 0 && $hours_diff <= 24) {
            // Returned within 1 day after due date
            $return_status = 'late';
        } else {
            // Returned more than 1 day after due date
    ...
    $overdue_hours = null;
    if ($return_status === 'overdue') {
        // Calculate hours beyond 1 day grace period
        $overdue_hours = $hours_diff - 24;
        error_log("Overdue Hours: $overdue_hours");
    }
