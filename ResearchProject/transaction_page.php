...
<body>
    ...
    <section class="container" style="padding: 2rem 0;">
      ...
      <div class="circulation-transaction-box">
        ...
        <div class="circulation-form">
          <div class="form-group" id="registeredUserSearchGroup">
          ...
          <div class="form-group" id="circulationDurationGroup" style="display: none;">
            ...
            <input type="number" id="circulationDuration" placeholder="e.g., 7" min="1" max ="7" style="width: 100%; padding: 0.6rem 0.8rem; border: 1px solid #cde; border-radius: 6px; font-size: 0.95rem; transition: border-color 0.3s;">
            <div id="circulationDurationError" style="color: #c33; font-size: 0.9rem; margin-top: 0.4rem; display: none;"></div>
          </div>
          ...
          <!-- CCT Success Modal -->
      <div id="cctSuccessModal" class="modal-overlay">
        <div class="modal" style="max-width: 520px; text-align: left;">
          <h2>Book Lent (CCT)</h2>
          <p id="cctSuccessMessage" style="color: #333; margin-bottom: 0.5rem; font-weight: 600;">Success</p>
          <div style="margin-bottom: 1rem; color: #555;">
            <p><strong>Book:</strong> <span id="cctBookTitle">—</span></p>
            <p><strong>Accession:</strong> <span id="cctAccession">—</span></p>
            <p><strong>Due Date:</strong> <span id="cctDueDate">—</span></p>
          </div>
          <div style="display:flex; justify-content:flex-end; gap:0.5rem;">
            <button onclick="closeCCTSuccessModal()" style="padding:0.6rem 1rem; border-radius:6px; border:1px solid #cde; background:#f3f3f3; cursor:pointer;">Close</button>
          </div>
        </div>
      </div>  
  ...
  <div id="rejectionModal" class="modal-overlay">
    <div class="modal" style="max-width: 600px;">
      ...
      <p style="color: #666; margin-bottom: 1.5rem;">Are you sure you want to reject this request? Please provide a reason (optional).</p>
      <textarea id="rejectionReasonInput" placeholder="Enter rejection reason..." style="width: 100%; padding: 0.75rem; border: 1px solid #ddd; border-radius: 6px; font-family: inherit; font-size: 0.95rem; resize: vertical; min-height: 100px; margin-bottom: 1.5rem;"></textarea>
      <div style="display: flex; gap: 1rem; justify-content: flex-end; margin-top: 2rem;">
        <button onclick="closeRejectionModal()" style="background: #f0f0f0; color: #333; padding: 0.65rem 1.5rem; border: none; border-radius: 6px; cursor: pointer; font-weight: 500;">Cancel</button>
        <button onclick="handleRejectionConfirm()" style="background: #d32f2f; color: white; padding: 0.65rem 1.5rem; border: none; border-radius: 6px; cursor: pointer; font-weight: 500;">Reject Request</button>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    function showRejectionModal() {
      document.getElementById('rejectionReasonInput').value = 'Your borrow request has not been approved at this time.';
    ...
    // CCT Success Modal functions
    function showCCTSuccessModal(bookTitle, accession, dueDate) {
      document.getElementById('cctBookTitle').textContent = bookTitle || '—';
      document.getElementById('cctAccession').textContent = accession || '—';
      document.getElementById('cctDueDate').textContent = dueDate || '—';
      document.getElementById('cctSuccessModal').classList.add('show');
    }

    function closeCCTSuccessModal() {
      document.getElementById('cctSuccessModal').classList.remove('show');
    }
        ...
        .then(data => {
          if (data.success && data.requests && data.requests.length > 0) {
            ...
            data.requests.forEach(request => {
              ...
              if (status === 'pending') {
                html += `
                  <button class="btn-approve" data-action="approve" onclick="openConfigureModalFromPending(${request.id}, &quot;${escapeHtml(request.book_title)}&quot;, &quot;${escapeHtml(request.user_name)}&quot;, &quot;${escapeHtml(request.author)}&quot;, ${request.book_id})">Configure</button>
                `;
              }
    ...
    function openConfigureModalFromPending(requestId, bookTitle, userName, author, bookId) {
      ...
      // No bookCopyId for pending requests since they haven't been borrowed yet
      loadBookCopiesForConfigure(bookId, null, bookTitle, userName, author);
    }

    function loadBookCopiesForConfigure(bookId, bookCopyId, bookTitle, userName, author, currentBorrowType, currentBorrowDays, currentBorrowSchedule) {
      // Prepare pre-selected buttons and show correct sub-options
      // Default to 'takehome' if no type is specified (for pending requests)
      let takehomeSelected = currentBorrowType === 'takehome' || (!currentBorrowType && !currentBorrowSchedule);
      let classroomSelected = currentBorrowType === 'classroom';
      
      const html = `
        <div class="book-info-card">
          <h3>${bookTitle} by ${author}</h3>
          <p><strong>Request By:</strong> ${userName}</p>
        </div>
      ...
      if (bookId) {
        loadAvailableCopies(bookId, bookCopyId);
      }
    }

    function loadAvailableCopies(bookId, bookCopyId) {
      console.log('loadAvailableCopies called with bookId:', bookId, 'bookCopyId:', bookCopyId);
      let url = `get_available_copies.php?book_id=${bookId}`;
      if (bookCopyId) {
        url += `&current_copy_id=${bookCopyId}`;
      }
      fetch(url)
        .then(response => {
            ...
            data.books.forEach(book => {
              ...
              html += `
                <div class="request-item" style="display: flex; justify-content: space-between; align-items: center; padding: 1rem; border: 1px solid #cde; border-radius: 8px; margin-bottom: 1rem; background: #f9fafb;">
                  <div class="request-info" onclick="openConfigureModal(${book.id}, &quot;${escapeHtml(book.book_title)}&quot;, &quot;${escapeHtml(book.user_name)}&quot;, &quot;${escapeHtml(book.author)}&quot;, ${book.book_id}, ${book.book_copy_id || 'null'}, ${book.borrow_type ? `'${book.borrow_type}'` : 'null'}, ${book.borrow_duration || 'null'}, '${book.borrow_schedule || 'am'}')" style="cursor: pointer; flex: 1;">
                    ...
                    <p style="margin: 0.25rem 0; font-size: 0.95rem; color: var(--muted);"><strong>Email:</strong> ${escapeHtml(book.user_email)}</p>
    ...
    function openConfigureModal(borrowId, bookTitle, userName, author, bookId, bookCopyId, currentBorrowType, currentBorrowDays, currentBorrowSchedule) {
      ...
      // Load available book copies and show modal with initial content
      loadBookCopiesForConfigure(bookId, bookCopyId, bookTitle, userName, author, currentBorrowType, currentBorrowDays, currentBorrowSchedule);
    }
    ...
    function saveBorrowConfiguration() {
      ...
      // Book copy selection is optional (in case no copies are available yet)
      const copySelect = document.getElementById('bookCopySelect');
      const bookCopyId = copySelect && copySelect.value ? copySelect.value : null;
      ...
      if (saveButton) {
        ...
        saveButton.innerHTML = '<span style="opacity: 0.7;">Processing...</span>';
          ...
          .then(response => {
            ...
            return response.text().then(text => {
              ...
              try {
                ...
                console.error('Invalid JSON response from handle_request.php:', text);
                parsed = { success: false, message: 'Invalid server response', raw: text };
              }
          ...
          .then(response => {
            ...
            return response.text().then(text => {
              try {
                ...
                console.error('Invalid JSON response from save_borrow_config.php:', text);
    ...
    function handleRequest(requestId, action) {
      // Find the request item and disable its buttons
      ...
      const actionButton = requestItem.querySelector(`button[data-action="${action}"]`);
      if (actionButton) {
        const originalText = actionButton.textContent;
        actionButton.innerHTML = '<span style="opacity: 0.7;">Processing...</span>';
      }
      ...
      fetch('handle_request.php', {
        ...
        .then(data => {
          if (data.success) {
            ...
            if (actionButton) {
              actionButton.innerHTML = action === 'approve' ? '✓ Approved' : '✓ Rejected';
              actionButton.style.background = '#2e7d32';
            }
            ...
            if (actionButton) {
              actionButton.innerHTML = action === 'approve' ? 'Approve' : 'Reject';
    ...
    function markAsReturned(borrowedBookId) {
      const confirmMsg = 'Are you sure you want to mark this book as returned?';
      showConfirmModal('Confirm Return', confirmMsg, () => {
        const formData = new FormData();
    ...
    function renderFilteredItems(type, items) {
      ...
      if (type === 'pending') {
        ...
        items.forEach(request => {
          ...
          const userName = escapeHtml(request.user_name);
          const bookCopy = escapeHtml(request.copy_number ? `Copy #${request.copy_number}` : 'N/A');
          const bookCopyInfo = request.accession_code ? `${bookCopy} (Accession: ${escapeHtml(request.accession_code)})` : bookCopy;
          const userEmail = request.user_email ? escapeHtml(request.user_email) : 'N/A';
          ...
          html += `
            <div class="request-item">
              ...
              <div class="request-actions">
                <button class="btn-configure" onclick="openConfigureModalFromPending(${request.id}, &quot;${bookTitle}&quot;, &quot;${userName}&quot;, &quot;${author}&quot;, ${request.book_id})">Configure</button>
              </div>
            </div>
          `;
        });
        ...
        items.forEach(book => {
          ...
          const userName = escapeHtml(book.user_name);
          const bookCopy = escapeHtml(book.copy_number ? `Copy #${book.copy_number}` : 'N/A');
          const bookCopyInfo = book.accession_code ? `${bookCopy} (Accession: ${escapeHtml(book.accession_code)})` : bookCopy;
          const userEmail = book.user_email ? escapeHtml(book.user_email) : 'N/A';
          ...
          let statusText = statusLabels[returnStatus] || (returnStatus ? returnStatus.charAt(0).toUpperCase() + returnStatus.slice(1) : 'Active');
          ...
          html += `
            <div class="request-item">
              <div class="request-info">
                ...
                <p><strong>Borrowed By:</strong> ${userName}</p>
                <p><strong>Borrow Date:</strong> ${new Date(book.borrow_date).toLocaleString()}</p>
                <p><strong>Due Date:</strong> ${new Date(book.due_date).toLocaleString()}</p>
    ...
    function updateUserSelectionDisplay() {
      // Always show registered user search group since guest is removed
    ...
    function updateCirculationDurationDisplay() {
      ...
      if (borrowType === 'takehome') {
        ...
        durationGroup.style.display = 'none';
        scheduleGroup.style.display = 'none';
        // ensure lend button state is correct after fetching last borrow
        setTimeout(() => validateCirculationDuration(), 250);
      } else {
        durationGroup.style.display = 'none';
        scheduleGroup.style.display = 'none';
        validateCirculationDuration();
    ...
    function validateCirculationDuration() {
      const input = document.getElementById('circulationDuration');
      const btn = document.querySelector('.btn-lend-direct');
      const err = document.getElementById('circulationDurationError');
      if (!input || !btn) return true;
      const val = (input.value || '').toString().trim();

      // If duration field is hidden or not required, clear error and enable button
      const borrowType = document.querySelector('input[name="circulationBorrowType"]:checked')?.value || '';
      if (borrowType !== 'takehome') {
      ...
      if (val === '') {
        err.textContent = 'Please enter borrow duration';
      ...
      if (!/^\d+$/.test(val)) {
        err.textContent = 'Duration must be a whole number';
        err.style.display = 'block';
        btn.disabled = true;
        return false;
      }
      ...
      if (num < 1) {
        err.textContent = 'Minimum is 1 day';
        err.style.display = 'block';
        btn.disabled = true;
        return false;
      }
      ...
      if (num > 7) {
        err.textContent = 'Maximum is 7 days';
        err.style.display = 'block';
        btn.disabled = true;
        return false;
      }
        ...
        .then(data => {
          if (data.success) {
            window.lastBorrowData = {
              ...
              book_copy_id: data.book_copy_id || null,
              accession_code: data.accession_code || null,
    ...
    function lendBookDirectly() {
      ...
      // For renew, admin does not need to select an accession; use last borrow data
      if (borrowType !== 'renew') {
        ...
        if (!window.lastBorrowData || !window.lastBorrowData.book_id) {
          showAlertModal('Error', 'Could not determine the user\'s last borrowed book. Please select a book or fetch last borrow first.');
          return;
        }
      }
      ...
      // Assign book id and copy id based on type
      if (borrowType === 'renew') {
        // For renew, use the book id from the user's last returned borrow but do NOT pick a specific copy
        borrowData.book_id = window.lastBorrowData.book_id;
      } else {
        borrowData.book_id = bookId;
        borrowData.book_copy_id = bookCopyId;
      }

      if (borrowType === 'takehome') {
        // validate duration field before proceeding
        if (!validateCirculationDuration()) {
          return;
        }
      ...
      // Show confirmation before lending (accession shown only when explicitly selected)
      const bookTitle = (window.selectedCirculationBook && window.selectedCirculationBook.title) || (window.lastBorrowData && window.lastBorrowData.book_title) || 'Unknown';
      ...
      // Store borrowData and userId for use in confirmation callback
      window.pendingBorrowData = borrowData;
      window.pendingUserId = userId;
      
      showConfirmModal('Confirm Book Lending', confirmMsg, () => {
        proceedWithLending(window.pendingUserId, window.pendingBorrowData);
      });
    }

    function proceedWithLending(userId, borrowData) {
      const formData = new FormData();
      formData.append('user_id', userId);
      formData.append('book_id', borrowData.book_id);
      formData.append('borrow_type', borrowData.borrow_type);
      if (typeof borrowData.book_copy_id !== 'undefined' && borrowData.book_copy_id !== null && borrowData.book_copy_id !== '') {
        formData.append('book_copy_id', borrowData.book_copy_id);
      }
      ...
      .then(response => {
        // Read full text for robust error reporting
      ...
      .then(({ status, text }) => {
        ...
        try {
          ...
          console.warn('Invalid JSON from lend_book_direct.php:', text);
        }

        console.log('lend_book_direct response status:', status, 'body:', data || text);

        if (status >= 200 && status < 300 && data && data.success) {
          const dueDate = new Date(data.due_date);
          const dueDateStr = dueDate.toLocaleString();
          const successTitle = (window.selectedCirculationBook && window.selectedCirculationBook.title) || (window.lastBorrowData && window.lastBorrowData.book_title) || 'Unknown';
          const successAccession = (window.selectedCirculationBook && window.selectedCirculationBook.accessionCode) || '—';
          showCCTSuccessModal(successTitle, successAccession, dueDateStr);
          // Reset form
          ...
          window.selectedCirculationBookId = null;
          window.selectedCirculationCopyId = null;
          ...
          const msg = (data && data.message) ? data.message : (text || 'Failed to lend book');
          showAlertModal('Error', `Error lending book (status ${status}): ${msg}`);
        }
      })
      .catch(error => {
        console.error('Fetch error:', error);
        showAlertModal('Error', 'Network error lending book: ' + error.message);
      });
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      // Wire duration input validation
      const durationInput = document.getElementById('circulationDuration');
      if (durationInput) {
        durationInput.addEventListener('input', validateCirculationDuration);
      }
