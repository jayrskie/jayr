...
try {
    ...
    if ($borrowRequest['user_id']) {
        ...
        $userName = $borrowRequest['guest_name'];
    }
    ...
    if ($action === 'approve') {
        ...
        $emailDueDate = null;
        if ($borrow_type === 'takehome') {
            $emailDueDate = date('F j, Y', strtotime('+' . $borrow_duration . ' days'));
        } else {
            if ($borrow_schedule === 'am') {
                $emailDueDate = date('F j, Y') . ' at 1:00 PM';
            } else {
                $emailDueDate = date('F j, Y') . ' at 7:00 PM';
            }
        }

        // Send approval email to user if they have an email
        if ($userEmail && $userName) {
            sendBorrowApprovalEmail($userEmail, $userName, $bookTitle, $bookAuthor, $emailDueDate, $borrow_type, $borrow_duration, $borrow_schedule, 'Online');
        }
