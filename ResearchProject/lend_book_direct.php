...
try {
    ...
    if ($user_type === 'registered') {
        ...
        if ($userData && $userData['email'] && $bookData) {
            ...
            $emailDueDate = null;
            if ($borrow_type === 'takehome') {
                $emailDueDate = $due_date->format('F j, Y');
            } else {
                if ($borrow_schedule === 'am') {
                    $emailDueDate = $due_date->format('F j, Y') . ' at 1:00 PM';
                } else {
                    $emailDueDate = $due_date->format('F j, Y') . ' at 7:00 PM';
                }
            }
            
            sendBorrowApprovalEmail($userData['email'], $userData['name'], $bookData['title'], $bookData['author'] ?? null, $emailDueDate, $borrow_type, $borrow_duration, $borrow_schedule, 'CCT');
        }
    }
