...
try {
    ...
    $user_id = intval($_GET['user_id']);
        ...
        // Get user's most recent returned borrow to use as template for renewal
        $query = 'SELECT 
                                bb.borrow_type,
                                bb.borrow_duration,
                                bb.borrow_schedule,
                            ...
                            WHERE br.user_id = ? AND bb.return_date IS NOT NULL
    ...
    if ($lastBorrow) {
        echo json_encode([
            ...
            'book_id' => $lastBorrow['book_id'],
            'book_copy_id' => $lastBorrow['book_copy_id']
        ]);
    } else {
        echo json_encode([
            'success' => false,
            'message' => 'No returned borrows found for this user'
        ]);
    }
